// assign each thread a block of continuous rows
// paralleled:
//      find pivot row
//      swap rows
//      normalize pivot row
...
void rows_swap(int work_row, int pivot_row, int thread_id) {
	for(int c = work_row + thread_id; c < N; c += thread_num) {
		double temp = matrix[work_row][c];
		matrix[work_row][c] = matrix[pivot_row][c];
...
void row_norm(int work_row, int thread_id, double den) {
    for(int c = work_row + thread_id; c < N; c += thread_num) {
		matrix[work_row][c] /= den;
	}
}
...
void *work_thread(void *lp) {
	...
	for(int work_row = 0; work_row < M; work_row++) {
        ...
        rows_swap(work_row, pivot_row, thread_id);
        pthread_barrier_wait(&barrier);

        // normalize work row
        double den = matrix[work_row][work_row];
        pthread_barrier_wait(&barrier);
        row_norm(work_row, thread_id, den);
        pthread_barrier_wait(&barrier);
...
//    gettimeofday(&finish, 0);
    ...
    matrix_write("v2_block_pthread_result");
